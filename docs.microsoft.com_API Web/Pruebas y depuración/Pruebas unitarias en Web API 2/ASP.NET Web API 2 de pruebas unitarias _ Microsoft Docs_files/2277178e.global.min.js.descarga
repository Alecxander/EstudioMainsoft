...
function trackSelectElementChange(awa) {
    function handleChange(event) {
        ...
        wedcs('ms.switcheventtime', new Date().getTime().toString(), 'ms.switcher', event.target.getAttribute(contentAttrs.name).replace(/^select-/, ''), 
        'ms.switchervalue', event.target.value);
    }
...
function trackPageFocus(awa) {
    ...
    function reportFocusChanged(event) {
        ...
        wedcs('ms.focuseventtime', new Date().getTime().toString(), 'ms.focusorblue', 
        event.type === 'focus' ? 'Focused' : 'Blured' 
        );
    }
    var timeout = 0;
...
function getRestApiBase() {
    return getMeta('api') || '/en-us/rest/api/rest-products.json';
}
var apiBase = 'https://docs.microsoft.com/api/';
var dhsApiBase = findDhsApiBase();
var restApiBase = getRestApiBase();
function fetchPlatform(platformId) {
    return platformId === 'rest'
        ? fetchWithTimeout(restApiBase)
            .then(function (response) { return response.json(); })
            .then(function (response) { return mapSiteSearchToPlatform(response, platformId); })["catch"](function () { return ({ platformId: platformId, displayName: platformId, families: [] }); })
...
function siteSearch(locale, dataSource, scope, term, skip, take) {
    ...
    if (scope !== '') {
        url += "&$filter=" + encodeURIComponent("scopes/any(t: t eq '" + scope + "')");
    }
    if (dataSource !== '') {
...
function fetchRestNamespaces(platform, moniker) {
    return fetchWithTimeout(restApiBase)
        .then(function (response) { return response.json(); })
        .then(function (response) { return mapSiteSearchToNamespaceResponse(response, moniker); });
}
...
function renderAuthentication(container) {
    return new Promise(function (resolve) {
        ...
        heading.textContent = loc.signIn;
...
function renderLoginButton(container, processTokens) {
    ...
    label.textContent = loc.loginAzure;
    container.appendChild(label);
    var loginButton = document.createElement('button');
    loginButton.setAttribute('type', 'button');
    loginButton.classList.add('login');
    loginButton.classList.add('is-small');
    loginButton.classList.add('signin-button');
    loginButton.textContent = loc.login;
...
function reportAuthorized() {
    ...
    wedcs('ms.azure-auth-login', 'authorized');
}
...
function renderBreakLine(container) {
...
function renderParam(container, bus, param, isHeaders, urlParams) {
        ...
        case 'enum':
            ...
            valueSelect.onchange = function () {
                ...
                if (valueSelect.textContent !== valueSelect.value) {
                    descLabel.textContent = valueSelect.value;
                }
            };
    ...
    if (param.isRequired) {
        ...
        valueLabel.classList.add('param-value-label');
        valueLabel.textContent = loc.required;
    ...
    descLabel.classList.add('param-desc');
    ...
    if (!isHeaders) {
        if (param.name === 'subscriptionId') {
            ...
            bus.subscribe(RestSubscriptionIdLoaded, function (event) {
                ...
                descLabel.textContent = subscriptions[0].subscriptionId;
                bus.publish(new RestParamValueChanged());
            });
        }
    }
    ...
    return { label: label, valueInput: valueInput, delButton: delButton, li: li, descLabel: descLabel };
}
function renderEmptyParam(container, bus, isHeaders) {
    ...
    return { nameInput: nameInput, valueInput: valueInput, addButton: addButton, li: li, descLabel: descLabel };
}
...
function renderHttpRequest(container, bus, request) {
    ...
    codeBlock.classList.add('is-small');
...
function tutorialPage() {
    ...
    window.addEventListener('hashchange', function () {
        ...
        changeSection(document$1.location.hash);
    });
...
var manageActionColumn = function (aHash, displayType, firstHashId, lastHashId) {
    ...
    switch (displayType) {
        case 'one-column':
            ...
            break;
        case 'two-column':
            if (aHash === firstHashId || aHash === lastHashId) {
                displayPageActions(aHash === lastHashId);
            }
            else {
...
var setFirstAndLastHashNames = function (element) {
    ...
    return {
        ...
        lastHashId: tocAnchors.pop().getAttribute('href')
    };
};
...
var Dialog =  (function () {
    ...
    Dialog.prototype.show = function (view, options) {
        ...
        if (options.modal) {
            ...
            document$1.querySelector('.mainContainer').style.filter = 'blur(3px)';
...
if (msDocs.data.brand === 'mooncake') {
    cookieConsent.then(function () { return loadLibrary(relativeToGlobal('js/wedcs.js')); });
}
